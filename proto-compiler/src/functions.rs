                ...
                .filter(|e| {
                    e.file_type().is_file()
                        && (e.path().file_stem().unwrap() == "PulsarApi"
                            || e.path().file_stem().unwrap() == "Function"
                            || e.path().file_stem().unwrap() == "Request"
                            || e.path().file_stem().unwrap() == "InstanceCommunication")
                })
                .map(|e| e.into_path())
                .collect(),
        );
...
/// Create pulsar.rs with library information
pub fn generate_pulsar_lib(prost_dir: &PathBuf, pulsar_lib_target: &PathBuf) {
    let file_names = WalkDir::new(prost_dir)
        .into_iter()
        .filter_map(|e| e.ok())
        .filter(|e| {
            e.file_type().is_file()
                && e.file_name().to_str().unwrap().starts_with("pulsar.")
